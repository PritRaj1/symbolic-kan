...
function prune(rng::AbstractRNG, m, ps, st; threshold=1e-3, mode="auto", active_neurons_id=nothing, verbose=true)
    ...
    for i in 1:size(st.act_scale, 1)
        if i < size(st.act_scale, 1) - 1
            push!(biases, ps.biases[Symbol("layer_$i")][:, active_neurons_id[i+1]])
        end
        ...
        push!(act_fcns_ps, ps_new)
        push!(act_fcns_st, st_new)
        ...
        push!(symbolic_ps, ps_new)
        push!(symbolic_fcns_st, st_new)
        @reset model_pruned.symbolic_fcns[i] = new_fcn
    end

    ps_pruned = (
        act_fcns_ps=act_fcns_ps,
        symbolic_fcns_ps=symbolic_ps,
        biases=biases
    )

    st_pruned = (
        act_fcns_st=act_fcns_st,
        symbolic_fcns_st=symbolic_fcns_st,
        acts = st.acts,
...
# Flux.@layer KAN_ trainable=(biases, act_fcns)
